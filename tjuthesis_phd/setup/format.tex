...
%  Supplementer:Jack Shen: congshen@tju.edu.cn        天津大学2015级智能与计算学部计算机科学与技术学院计算机应用技术专业博士生
...
\makeatletter
\renewcommand\normalsize{
  \@setfontsize\normalsize{12pt}{15pt} % 小四对应12pt,1.25倍行距
...
%             {\hspace{.5em}\titlerule*[10pt]{.}\sihao\contentspage}
...
\setcounter{tocdepth}{2}
\titlecontents{chapter}[0em]{\vspace{0pt}\xiaosi\hei}%
             {\prechaptername\CJKnumber{\thecontentslabel}\postchaptername\quad}{} %
             {\hspace{.5em}\titlerule*[3.5pt]{$\cdot$}\xiaosi\contentspage}
...
\fancypagestyle{plain}{% 初始化plain风格
    \fancyhf{}%
    \fancyhead[CO]{\song\wuhao \leftmark}
    \fancyhead[CE]{\song\wuhao \@cheading}
...
\fancypagestyle{plain2}{% 设置开章页页眉页脚风格
    \fancyhf{}%
    \fancyhead[CO]{\song\wuhao \leftmark}
    \fancyhead[CE]{\song\wuhao \@cheading}
    \fancyfoot[C]{\song\xiaowu ~\thepage~ } %%首页页脚格式
...
\clearpage
\markboth{摘~要}{摘~要}
%\addcontentsline{toc}{chapter}{摘~要} %加上则意味着目录里也要出现
\chapter*{\centering\erhao{\bfseries\song 摘\qquad 要}}
\thispagestyle{only_foot}%用来去掉页眉
\setcounter{page}{1}
\song\defaultfont
\@cabstract
\pagestyle{only_foot}
...
%\addcontentsline{toc}{chapter}{ABSTRACT} %加上则意味着目录里也要出现
